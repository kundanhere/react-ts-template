...
function Sortable<T>(props: SortableProps<T>) {
  ...
  const announcements: Announcements = React.useMemo(
    () => ({
      onDragStart({ active }) {
        ...
        const currentIndex = (active.data.current?.sortable.index ?? 0) + 1;
        return `Grabbed sortable item "${activeValue}". Current position is ${currentIndex} of ${value.length}. Use arrow keys to move, space to drop.`;
      },
  ...
  const screenReaderInstructions: ScreenReaderInstructions =
    React.useMemo(() => {
      let arrowKeysDesc = "arrow";
      if (orientation === "vertical") {
        arrowKeysDesc = "up and down";
      } else if (orientation === "horizontal") {
        arrowKeysDesc = "left and right";
      }
      return {
        draggable: `
          To pick up a sortable item, press space or enter.
          While dragging, use the ${arrowKeysDesc} keys to move the item.
...
function SortableItem(props: SortableItemProps) {
  ...
  const composedRef = useComposedRefs(ref as any, (node) => {
    if (disabled) return;
    setNodeRef(node as any);
    if (asHandle) setActivatorNodeRef(node as any);
  });
...
function SortableItemHandle(props: SortableItemHandleProps) {
  ...
  const composedRef = useComposedRefs(ref as any, (node) => {
    ...
    itemContext.setActivatorNodeRef(node as any);
  });
